 ...
 *
 * Fixed from the original: the two separate `damaged` functions have been merged (JS
 * silently keeps only the LAST declaration of a function with a given name, so the
 * showcase-start behavior in the first one was never actually running), and the
 * T_BLAST attack is now null-safe if the NPC has no current attack target.
 */
"use strict";
...
// Single damaged handler: starts the showcase on first hit, and once transformed
// (T_BLAST already fired at least once this cycle isn't tracked separately here --
// retaliation just fires every hit taken, same as the original bottom function did).
function damaged(event) {
    startShow(event.npc);
    var attacker = event.source;
    if (typeof XenoPixels === "undefined" || attacker == null) return;
    XenoPixels.fireTechnique(event.npc, "supernova_cooler", attacker, 100, "FF00F2");
}
...
function timer(event) {
    ...
    if (event.id == T_SSJ1) {
        ...
        if (target != null) {
            n.say("Take this!");
            XenoPixels.fireTechnique(n, "supernova_cooler", target, 100, "FF00F2");
        } else {
            n.say("...where did they go?");
        }

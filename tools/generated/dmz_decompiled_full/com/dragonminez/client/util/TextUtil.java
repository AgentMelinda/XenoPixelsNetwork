...
public class TextUtil {
   ...
   public static FormattedCharSequence overrideColor(FormattedCharSequence sequence, int color) {
      return sink -> sequence.accept((index, style, codePoint) -> sink.accept(index, style.withColor(TextColor.fromRgb(color)), codePoint));
   }
   ...
   public static void drawStringWithBorder(GuiGraphics graphics, Font font, String text, int x, int y, int textColor) {
      drawStringWithBorder(graphics, font, text, x, y, textColor, -16777216);
   }
   ...
   public static void drawCenteredStringWithBorder(
      GuiGraphics graphics, Font font, FormattedCharSequence text, int centerX, int y, int textColor, int borderColor
   ) {
      int x = centerX - font.width(text) / 2;
      drawStringWithBorder(graphics, font, text, x, y, textColor, borderColor);
   }
   ...
   public static void drawCenteredStringWithBorder(GuiGraphics graphics, Font font, String text, int centerX, int y, int textColor) {
      drawCenteredStringWithBorder(graphics, font, text, centerX, y, textColor, -16777216);
   }
   ...
   public static void renderAdvancedTooltip(
      ...
      if (title != null || description != null && !description.isEmpty() || extras != null && !extras.isEmpty()) {
         ...
         if (extras != null && !extras.isEmpty()) {
            ...
            for (Component cx : extras) {
               String raw = cx.getString();
               if (raw.contains("\n")) {
                  for (String part : raw.split("\n")) {
                     for (FormattedCharSequence wrapped : font.split(Component.literal(part).setStyle(cx.getStyle()), allowedMaxWidth)) {
                        components.add(new CenteredTextComponent(wrapped));
                     }
                  }
               } else {
                  for (FormattedCharSequence wrapped : font.split(cx, allowedMaxWidth)) {
                     components.add(new CenteredTextComponent(wrapped));
                  }
               }
            }
   ...
   public static boolean overScrollBar(double mouseX, double mouseY, int barX, int barWidth, int trackY, int trackHeight) {
      return mouseX >= (double)(barX - 4) && mouseX <= (double)(barX + barWidth + 4) && mouseY >= (double)trackY && mouseY <= (double)(trackY + trackHeight);
   }

   public static float scrollFromBar(double mouseY, int trackY, int trackHeight, float maxScroll) {
      if (trackHeight > 0 && !(maxScroll <= 0.0F)) {
         float percent = Mth.clamp((float)(mouseY - (double)trackY) / (float)trackHeight, 0.0F, 1.0F);
   ...
   public static void renderScrollableText(
      GuiGraphics graphics, Font font, List<String> lines, int x, int y, int width, int height, float currentScroll, float maxScroll, int color
   ) {
      renderScrollableText(graphics, font, lines, x, y, width, height, currentScroll, maxScroll, color, Style.EMPTY);
   }
   ...
   public static void renderScrollableText(
      GuiGraphics graphics, Font font, List<String> lines, int x, int y, int width, int height, float currentScroll, float maxScroll, int color, Style style
   ) {
      int lineHeight = 9 + 2;
      ...
      for (int i = 0; i < lines.size(); i++) {
         ...
         if (lineY + (float)lineHeight >= (float)y + currentScroll && lineY <= (float)(y + viewHeight) + currentScroll) {
            drawStringWithBorder(graphics, font, Component.literal(lines.get(i)).setStyle(style), x, (int)lineY, color);
         }
      }
      ...
      if (maxScroll > 0.0F) {
         ...
         graphics.fill(scrollBarX, y, scrollBarX + 3, y + height, -13421773);
         float scrollPercent = maxScroll == 0.0F ? 0.0F : currentScroll / maxScroll;
         int indicatorHeight = Math.max(10, (int)((float)viewHeight / (float)totalContentHeight * (float)height));

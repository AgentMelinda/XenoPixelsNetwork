...
public final class BetaAccessVerification {
   ...
   public static String buildVerificationUrl(String username) {
      if (!isValidMinecraftUsername(username)) {
         ...
         String baseUrl = System.getProperty("dragonminez.betaAccessUrl", "https://downloads.dragonminez.com/beta-access/start").trim();
   ...
   public static boolean isBetaAccessDisconnect(String reason) {
      if (reason != null && !reason.isBlank()) {
         String normalizedReason = FORMAT_CODE.matcher(reason).replaceAll("").toLowerCase(Locale.ROOT);
         return normalizedReason.contains("[dragonmine z]") && normalizedReason.contains("not allowed to play this beta/alpha version");
      } else {
         return false;
      }
   }

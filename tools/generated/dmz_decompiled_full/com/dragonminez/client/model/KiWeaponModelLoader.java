...
public class KiWeaponModelLoader {
      ...
      if (resource.isEmpty()) {
         ...
         try {
            ...
            try (InputStreamReader reader = new InputStreamReader(resource.get().open(), StandardCharsets.UTF_8)) {
               ...
               float[] defaultPivot = new float[]{0.0F, 24.0F, 0.0F};
               ...
               for (JsonElement boneElem : bones) {
                  ...
                  float[] parentJavaRot = new float[]{0.0F, 0.0F, 0.0F};
                  ...
                  float[] rot = readVec(bone, "rotation", 0.0F, 0.0F, 0.0F);
                  CubeListBuilder cubes = CubeListBuilder.create();
                  int rotatedCubeIndex = 0;
                  List<JsonObject> rotatedCubes = new ArrayList<>();
                  if (bone.has("cubes")) {
                  ...
                  if (parentJavaRot[0] != 0.0F || parentJavaRot[1] != 0.0F || parentJavaRot[2] != 0.0F) {
                     float rx = -parentJavaRot[0];
                     float cx = (float)Math.cos((double)rx);
                     float sx = (float)Math.sin((double)rx);
                     float dy1 = dy * cx - dz * sx;
                     float dz1 = dy * sx + dz * cx;
                     float ry = -parentJavaRot[1];
                     float cy = (float)Math.cos((double)ry);
                     float sy = (float)Math.sin((double)ry);
                     float dx1 = dx * cy + dz1 * sy;
                     float dz2 = -dx * sy + dz1 * cy;
                     dz = dz2;
                     float rz = -parentJavaRot[2];
                     float cz = (float)Math.cos((double)rz);
                     float sz = (float)Math.sin((double)rz);
                     float dx2 = dx1 * cz - dy1 * sz;
                     float dy2 = dx1 * sz + dy1 * cz;
                     dx = dx2;
                     dy = dy2;
                  }

                  float jRotX = (float)Math.toRadians((double)rot[0]);
                  float jRotY = (float)Math.toRadians((double)(-rot[1]));
                  float jRotZ = (float)Math.toRadians((double)(-rot[2]));
                  ...
                  for (JsonObject cube : rotatedCubes) {
                     ...
                     float[] cubeRot = readVec(cube, "rotation", 0.0F, 0.0F, 0.0F);
                     CubeListBuilder rotated = CubeListBuilder.create();
                     addCube(rotated, cube, cubePivot);
                     part.addOrReplaceChild(
                        name + "_r" + rotatedCubeIndex++,
                        rotated,
                        PartPose.offsetAndRotation(
                           cubePivot[0] - pivot[0],
                           pivot[1] - cubePivot[1],
                           cubePivot[2] - pivot[2],
                           (float)Math.toRadians((double)cubeRot[0]),
                           (float)Math.toRadians((double)(-cubeRot[1])),
                           (float)Math.toRadians((double)(-cubeRot[2]))
                        )
                     );
                  }
               ...
               var50 = LayerDefinition.create(mesh, texWidth, texHeight).bakeRoot();
            }

            return var50;
         } catch (Exception var49) {
            LogUtil.error(Env.CLIENT, "Failed to load Ki weapon model: " + type, var49);
            return null;
         }
      }
   }
   ...
   private static void addCube(CubeListBuilder builder, JsonObject cube, float[] pivot) {
      float[] origin = readVec(cube, "origin", 0.0F, 0.0F, 0.0F);
      float[] size = readVec(cube, "size", 0.0F, 0.0F, 0.0F);
      int[] uv = readBoxUv(cube);
      builder.texOffs(uv[0], uv[1])
         .addBox(origin[0] - pivot[0], pivot[1] - (origin[1] + size[1]), origin[2] - pivot[2], size[0], size[1], size[2], CubeDeformation.NONE);
   }

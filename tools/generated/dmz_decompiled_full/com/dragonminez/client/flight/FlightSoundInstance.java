...
public class FlightSoundInstance extends AbstractTickableSoundInstance {
   ...
   public void tick() {
      this.time++;
      if (!this.player.isRemoved() && this.isFlying(this.player)) {
         ...
         float speed = 0.0F;
         if ((double)speedSqr >= 1.0E-7) {
            speed = (float)Math.sqrt((double)speedSqr);
         }
         ...
         if (speedPct < 0.33F) {
            ...
            float range = (speedPct - 0.33F) / 0.16999999F;
            targetVolume = Mth.lerp(range, 0.1F, 0.4F);
            targetPitch = 1.0F;
         } else if (speedPct < 0.75F) {
            float range = (speedPct - 0.5F) / 0.25F;
   ...
   private boolean isFlying(LocalPlayer player) {
      Optional<StatsData> stats = StatsProvider.get(StatsCapability.INSTANCE, player).resolve();

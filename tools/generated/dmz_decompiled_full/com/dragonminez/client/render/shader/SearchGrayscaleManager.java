...
public final class SearchGrayscaleManager {
   private static final ResourceLocation EFFECT = ResourceLocation.fromNamespaceAndPath("dragonminez", "shaders/post/kisense_grayscale.json");
      ...
      if (mc.level != null) {
         ...
         if (chain == null) {
            try {
               ...
               LogUtil.error(Env.CLIENT, "Failed to load ki sense grayscale shader", var7);
               chain = null;
               return;
            }
         }
         ...
         if (width != lastWidth || height != lastHeight) {
            chain.resize(width, height);
            if (depthHolder != null) {
               depthHolder.resize(width, height, Minecraft.ON_OSX);
            }
         ...
         if (preserveDepth) {
            depthHolder.copyDepthFrom(main);
         }
         ...
         if (preserveDepth) {
            main.copyDepthFrom(depthHolder);
         }

...
public class KiSpiralMesh {
   ...
   private static final LinkedHashMap<KiSpiralMesh.Key, VertexBuffer> CACHE = new LinkedHashMap<KiSpiralMesh.Key, VertexBuffer>(16, 0.75F, true) {
      @Override
      protected boolean removeEldestEntry(Entry<KiSpiralMesh.Key, VertexBuffer> eldest) {
         if (this.size() > 8) {
   ...
   public static VertexBuffer get(
      int type, float size, float length, float radius, float tubeRadius, float tubeRadius2, float twistsPerBlock, boolean doubleHelix
   ) {
      KiSpiralMesh.Key key = new KiSpiralMesh.Key(type, Math.round(length * 4.0F), Math.round(size * 8.0F));
   ...
   private static VertexBuffer build(float length, float radius, float tubeRadius, float tubeRadius2, float twistsPerBlock, boolean doubleHelix) {
      ...
      int rings = Math.max(2, Math.min(3000, (int)(length * 24.0F)));
      float k = (float)((Math.PI * 2) * (double)twistsPerBlock);
   ...
   private static void appendTube(BufferBuilder builder, float length, float radius, float tubeRadius, float k, int rings, float phase) {
      float step = length / (float)rings;
      ...
      for (int i = 0; i < rings - 1; i++) {
         float z0 = (float)i * step;
         float z1 = (float)(i + 1) * step;
         ...
         float lz0 = (float)(i % 6) / 6.0F;
         Vector3f sideA = new Vector3f(side);
         Vector3f upA = new Vector3f(up);
         helixFrame(z1, radius, k, phase, tangent, side, up, ref);
         float lz1 = (float)((i + 1) % 6) / 6.0F;

         for (int j = 0; j < 8; j++) {
            float a0 = (float)((Math.PI * 2) * (double)j / 8.0);
            float a1 = (float)((Math.PI * 2) * (double)(j + 1) / 8.0);
            float c0 = (float)Math.cos((double)a0);
            float s0 = (float)Math.sin((double)a0);
            float c1 = (float)Math.cos((double)a1);
            float s1 = (float)Math.sin((double)a1);
   ...
   private static void helixCenter(Vector3f out, float radius, float k, float z, float phase) {
      float a = k * z + phase;
      out.set(-radius * (float)Math.sin((double)a), radius * (float)Math.cos((double)a), z);
   }

   private static void helixFrame(float z, float radius, float k, float phase, Vector3f tangent, Vector3f side, Vector3f up, Vector3f ref) {
      float a = k * z + phase;
      tangent.set(-radius * k * (float)Math.cos((double)a), -radius * k * (float)Math.sin((double)a), 1.0F).normalize();
   ...
   private static void ringPoint(Vector3f outPos, Vector3f outNormal, Vector3f center, Vector3f side, Vector3f up, float cos, float sin, float tubeRadius) {
      outNormal.set(side.x * cos + up.x * sin, side.y * cos + up.y * sin, side.z * cos + up.z * sin);
      outPos.set(center.x + outNormal.x * tubeRadius, center.y + outNormal.y * tubeRadius, center.z + outNormal.z * tubeRadius);
   }

   private static void vertex(BufferBuilder builder, Vector3f pos, Vector3f normal, float localX, float localY, float localZ, float u) {
   ...
   private static record Key(int type, int lengthBucket, int sizeBucket) {
   }
}

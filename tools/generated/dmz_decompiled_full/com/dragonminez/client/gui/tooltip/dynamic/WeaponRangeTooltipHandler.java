...
public class WeaponRangeTooltipHandler {
   ...
   public static boolean appendWeaponAttributes(WeaponAttributes attributes, Consumer<Component> tooltip) {
      if (attributes != null && attributes.attackRange() > 0.0) {
         ...
         Component rawAttrDesc = Component.translatable(((Attribute)Attributes.ENTITY_INTERACTION_RANGE.value()).getDescriptionId());
         Component attrDescNoIcon = IconUtil.getAttributeNameWithoutIcon(rawAttrDesc);
         Component coloredStat = Component.translatable("attribute.modifier.equals.0", new Object[]{FORMAT.format(totalRange), attrDescNoIcon})
            .withStyle(AttributeTooltipHandler.BASE_COLOR);

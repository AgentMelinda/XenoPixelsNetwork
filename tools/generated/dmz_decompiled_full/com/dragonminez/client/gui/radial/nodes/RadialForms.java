...
public final class RadialForms {
   ...
   public static List<RadialNode> stackForms(StatsData stats) {
      ...
      if (groups != null) {
         for (String group : groups.keySet()) {
            if (ConfigManager.getSkillsConfig().isSkillAllowedForRace(group, race)) {
      ...
      if (groups != null) {
         for (String group : groups.keySet()) {
            ...
            if (config != null) {
               ...
               if (typeFilter.test(type) && ConfigManager.getSkillsConfig().isSkillAllowedForRace(TransformationsHelper.getSkillNameForType(type), race)) {
   ...
   private static RadialNode buildGroupHead(StatsData stats, String race, String group, List<String> formNames, String parentCategoryKey, boolean stack) {
      if (formNames != null && !formNames.isEmpty()) {
         ...
         for (int i = 1; i < formNames.size(); i++) {
            rest.add(new FormSelectNode(race, group, formNames.get(i), stack));
         }
   ...
   private static List<RadialNode> orderAndCap(String categoryKey, List<RadialNode> out) {
      List<RadialNode> var2 = new ArrayList<>(RadialLayoutStore.applyOrder(categoryKey, out));
      return capWithMore(categoryKey, var2);
   }
   ...
   private static List<RadialNode> capWithMore(String categoryKey, List<RadialNode> all) {
      if (all.size() <= 5) {
         ...
         List<RadialNode> head = new ArrayList<>(all.subList(0, 4));

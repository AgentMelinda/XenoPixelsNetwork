...
public class SacredKaiPlacedFeatures {
   ...
   public static void bootstrap(BootstrapContext<PlacedFeature> context) {
      ...
      register(
         context,
         GRASS_PATCH_PLACED,
         configured.getOrThrow(SacredKaiConfiguredFeatures.GRASS_PATCH),
         surfaceSpread(NoiseThresholdCountPlacement.of(-0.8F, 5, 10))
      );
      register(
         context,
         FLOWERS_PLACED,
         configured.getOrThrow(SacredKaiConfiguredFeatures.FLOWERS),
      ...
      register(
         context,
         SAND_PATCH_PLACED,
         configured.getOrThrow(SacredKaiConfiguredFeatures.SAND_PATCH),
         List.of(RarityFilter.onAverageOnceEvery(10), InSquarePlacement.spread(), HeightmapPlacement.onHeightmap(Types.OCEAN_FLOOR_WG), BiomeFilter.biome())
      );
      register(context, ROCK_CLUSTER_PLACED, configured.getOrThrow(SacredKaiConfiguredFeatures.ROCK_CLUSTER), surfaceRarity(3));
      register(context, ROCK_CLUSTER_SPARSE_PLACED, configured.getOrThrow(SacredKaiConfiguredFeatures.ROCK_CLUSTER), surfaceRarity(8));
      register(context, GRASSY_PEAK_PLACED, configured.getOrThrow(SacredKaiConfiguredFeatures.GRASSY_PEAK), surfaceRarity(40));
      register(context, GRASSY_CLIFF_PLACED, configured.getOrThrow(SacredKaiConfiguredFeatures.GRASSY_CLIFF), surfaceRarity(32));
      register(context, KARST_PILLAR_PLACED, configured.getOrThrow(OverworldConfiguredFeatures.KARST_PILLAR_KEY), surfaceRarity(45));
      register(context, STONE_SPIKE_PLACED, configured.getOrThrow(OverworldConfiguredFeatures.STONE_SPIKE_KEY), surfaceRarity(90));
      register(
         context,
         WATER_LAKE_PLACED,
         configured.getOrThrow(SacredKaiConfiguredFeatures.WATER_LAKE),
   ...
   private static List<PlacementModifier> surfaceSpread(PlacementModifier count) {
      return List.of(count, InSquarePlacement.spread(), HeightmapPlacement.onHeightmap(Types.WORLD_SURFACE_WG), BiomeFilter.biome());
   }

   private static List<PlacementModifier> surfaceRarity(int onceEvery) {
      return List.of(
         RarityFilter.onAverageOnceEvery(onceEvery), InSquarePlacement.spread(), HeightmapPlacement.onHeightmap(Types.WORLD_SURFACE_WG), BiomeFilter.biome()
      );
   }

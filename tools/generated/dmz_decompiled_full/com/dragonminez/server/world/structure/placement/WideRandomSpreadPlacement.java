...
public class WideRandomSpreadPlacement extends RandomSpreadStructurePlacement {
   public static final MapCodec<WideRandomSpreadPlacement> CODEC = RecordCodecBuilder.mapCodec(
      instance -> placementCodec(instance)
            .and(RandomSpreadType.CODEC.optionalFieldOf("spread_type", RandomSpreadType.LINEAR).forGetter(RandomSpreadStructurePlacement::spreadType))
   ...
   public WideRandomSpreadPlacement(
      Vec3i locateOffset,
      ...
      super(locateOffset, frequencyReductionMethod, frequency, salt, exclusionZone, 1875, 750, spreadType);
   }
   ...
   public ChunkPos getPotentialStructureChunk(long seed, int chunkX, int chunkZ) {
      int spacing = this.spacing();
      int range = spacing - this.separation();
   ...
   protected boolean isPlacementChunk(ChunkGeneratorStructureState structureState, int x, int z) {
      if (!ConfigManager.getServerConfig().getWorldGen().getGenerateCustomStructures()) {
         ...
         ChunkPos pos = this.getPotentialStructureChunk(structureState.getLevelSeed(), x, z);
         return pos.x == x && pos.z == z;
      }
   }

   public StructurePlacementType<?> type() {
      return (StructurePlacementType<?>)MainStructurePlacements.WIDE_RANDOM_SPREAD.get();
   }
}

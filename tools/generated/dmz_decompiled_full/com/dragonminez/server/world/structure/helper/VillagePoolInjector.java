...
public class VillagePoolInjector {
   public static void injectAll(MinecraftServer server) {
      ...
      Registry<StructureProcessorList> procs = server.registryAccess().registryOrThrow(Registries.PROCESSOR_LIST);
   ...
   private static void inject(Registry<StructureTemplatePool> pools, Holder<StructureProcessorList> processors, String poolId, String pieceId, int weight) {
      StructureTemplatePool pool = (StructureTemplatePool)pools.get(ResourceLocation.parse(poolId));
      if (pool == null) {
         LogUtil.warn(Env.SERVER, "VillagePoolInjector: pool '" + poolId + "' not found, skipping '" + pieceId + "'.");
      } else {
         StructurePoolElement piece = (StructurePoolElement)StructurePoolElement.single(pieceId, processors).apply(Projection.RIGID);

...
public class TallJigsawStructure extends Structure {
   public static final MapCodec<TallJigsawStructure> CODEC = RecordCodecBuilder.mapCodec(
      instance -> instance.group(
               ...
               Types.CODEC.optionalFieldOf("project_start_to_heightmap").forGetter(s -> s.projectStartToHeightmap),
   ...
   public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
      ...
      if (this.projectStartToHeightmap.isPresent()) {
         Types heightmap = this.projectStartToHeightmap.get();
         if (this.minStartY > Integer.MIN_VALUE) {
            int bestH = Integer.MIN_VALUE;

            for (int dx = 0; dx < 16; dx += 4) {
               for (int dz = 0; dz < 16; dz += 4) {
                  int h = context.chunkGenerator().getFirstFreeHeight(x + dx, z + dz, heightmap, context.heightAccessor(), context.randomState());
                  if (h > bestH) {
                     bestH = h;
                     sx = x + dx;
                     sz = z + dz;
         ...
         startY = offset;
         if (this.minStartY > Integer.MIN_VALUE && offset < this.minStartY) {
            return Optional.empty();
         }
      }
   ...
   public StructureType<?> type() {
      return (StructureType<?>)MainStructureTypes.TALL_JIGSAW.get();
   }
}

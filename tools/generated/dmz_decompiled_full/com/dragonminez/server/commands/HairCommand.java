...
public class HairCommand {
   public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("dmzhair")
                  .requires(source -> DMZPermissions.check(source, DMZPermissions.HAIR_SELF, DMZPermissions.HAIR_OTHERS)))
               .then(
                  ((LiteralArgumentBuilder)Commands.literal("resync")
                        .executes(ctx -> resync((CommandSourceStack)ctx.getSource(), List.of(((CommandSourceStack)ctx.getSource()).getPlayerOrException()))))
                     .then(
                        ((RequiredArgumentBuilder)Commands.argument("targets", EntityArgument.players())
                           ...
                           .executes(ctx -> resync((CommandSourceStack)ctx.getSource(), EntityArgument.getPlayers(ctx, "targets")))
                     )
               ))
            .then(
               ((LiteralArgumentBuilder)Commands.literal("reset")
                     .executes(ctx -> reset((CommandSourceStack)ctx.getSource(), List.of(((CommandSourceStack)ctx.getSource()).getPlayerOrException()))))
                  .then(
                     ((RequiredArgumentBuilder)Commands.argument("targets", EntityArgument.players())
                           .requires(source -> DMZPermissions.hasPermission(source, DMZPermissions.HAIR_OTHERS)))
                        .executes(ctx -> reset((CommandSourceStack)ctx.getSource(), EntityArgument.getPlayers(ctx, "targets")))
                  )
            )
      );
   }
   ...
   private static int resync(CommandSourceStack source, Collection<ServerPlayer> targets) {
      ...
      source.sendSuccess(() -> Component.translatable("command.dragonminez.hair.resync", new Object[]{finalCount}), true);
      return count;
   }
   ...
   private static int reset(CommandSourceStack source, Collection<ServerPlayer> targets) {
      ...
      for (ServerPlayer player : targets) {
         ...
         if (data != null) {
            ...
            if (config != null) {
               ...
               if (config.getDefaultHairColor() != null) {
      ...
      int finalCount = count;
      source.sendSuccess(() -> Component.translatable("command.dragonminez.hair.reset", new Object[]{finalCount}), true);
      return count;
   }
}

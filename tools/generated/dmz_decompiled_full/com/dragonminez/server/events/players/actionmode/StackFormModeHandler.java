      ...
      if (nextForm == null) {
         ...
         if (data.getCharacter().hasActiveForm()) {
            ...
            if (activeFormData != null) {
               ...
               String stackGroup = data.getCharacter().hasActiveStackForm()
      ...
      if (nextForm != null) {
         if (data.getCharacter().hasActiveForm()) {
            ...
            if (activeFormData != null) {
               ...
               String nextGroup = data.getCharacter().hasActiveStackForm()
                  ? data.getCharacter().getActiveStackFormGroup()
                  : data.getCharacter().getSelectedStackFormGroup();
         ...
         int energyCost = (int)((double)data.getMaxEnergy() * nextForm.getEnergyDrain());
         int staminaCost = (int)((double)data.getMaxStamina() * nextForm.getStaminaDrain());
         int healthCost = (int)((double)data.getMaxHealth() * nextForm.getHealthDrain());
         ...
         if (hasEnoughEnergy && hasEnoughStamina && hasEnoughHealth) {
            ...
            if ("kaioken".equalsIgnoreCase(group)) {
               player.level()
                  .playSound(null, player.getX(), player.getY(), player.getZ(), (SoundEvent)MainSounds.STACK_FORM.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
            }
            ...
            Component translatedStackFormName = Component.translatable(
               "race.dragonminez.stack.form." + data.getCharacter().getSelectedStackFormGroup() + "." + nextForm.getName()
            );
            Component fullFormName;
            if (data.getCharacter().getActiveForm() != null && !data.getCharacter().getActiveForm().isEmpty()) {
               Component translatedFormName = Component.translatable(
                  "race.dragonminez."
                     + data.getCharacter().getRace()
                     + ".form."
               ...
               fullFormName = Component.empty().append(translatedStackFormGroup).append(Component.literal(" ")).append(translatedStackFormName);
            }

            player.sendSystemMessage(Component.translatable("message.dragonminez.transformation", new Object[]{fullFormName}), true);

...
public class FusionLogic {
   public static boolean executeMetamoru(ServerPlayer leader, ServerPlayer partner, StatsData lData, StatsData pData) {
      if (!lData.getStatus().isAndroidUpgraded() && !pData.getStatus().isAndroidUpgraded()) {
         if (!lData.getCharacter().getRaceName().equals(pData.getCharacter().getRaceName())) {
            ...
            if (threshold > 0.0) {
               double diff = (double)Math.abs(lvl1 - lvl2) / (double)Math.max(lvl1, lvl2);
               if (diff > threshold) {
            ...
            if (!lData.getStatus().isFused()
               && !pData.getStatus().isFused()
               && lData.getStatus().getFusionPartnerUUID() == null
               && pData.getStatus().getFusionPartnerUUID() == null) {
               ...
               if (((DMZEvent.FusionEvent)NeoForge.EVENT_BUS.post(event)).isCanceled()) {
                  return false;
               } else {
                  applyFusion(leader, partner, lData, pData, "METAMORU", lvl1, lvl2);
                  lData.getStatus().setFusionTimer(finalDuration);
                  leader.addEffect(new MobEffectInstance(MainEffects.FUSED, finalDuration, 0, false, false));
                  partner.addEffect(new MobEffectInstance(MainEffects.FUSED, finalDuration, 0, false, false));
                  leader.displayClientMessage(Component.translatable("message.dragonminez.fusion.success", new Object[]{partner.getDisplayName()}), true);
                  partner.displayClientMessage(Component.translatable("message.dragonminez.fusion.success", new Object[]{leader.getDisplayName()}), true);
                  return true;
               }
            } else {
               return false;
   ...
   public static void executePothala(ServerPlayer leader, ServerPlayer partner, StatsData lData, StatsData pData) {
      if (!lData.getStatus().isFused()
         ...
         if (!((DMZEvent.FusionEvent)NeoForge.EVENT_BUS.post(event)).isCanceled()) {
            ...
            leader.displayClientMessage(Component.translatable("message.dragonminez.fusion.success", new Object[]{partner.getDisplayName()}), true);
            partner.displayClientMessage(Component.translatable("message.dragonminez.fusion.success", new Object[]{leader.getDisplayName()}), true);
            leader.level().playSound(null, leader.getX(), leader.getY(), leader.getZ(), (SoundEvent)MainSounds.FUSION.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
      ...
      if (data.getStatus().isFused() || data.getStatus().getFusionPartnerUUID() != null) {
         ...
         if (leaderData != null) {
            ...
            if (original != null && !original.isEmpty()) {
               leaderData.getCharacter().loadAppearance(original);
            }
            ...
            if (appliesCd) {
               leaderData.getCooldowns().addCooldown("FusionCooldown", fusionCdTicks);
            }

            clearFusionState(leaderData);
            if (leaderRef != null) {
               if (leaderRef.hasEffect(MainEffects.FUSED)) {
                  leaderRef.removeEffect(MainEffects.FUSED);
               }

               if (appliesCd) {
                  leaderRef.addEffect(new MobEffectInstance(MainEffects.FUSION_CD, fusionCdTicks, 0, false, false, true));
               }
         ...
         if (partnerData != null) {
            int fusionCdTicksx = ConfigManager.getServerConfig().getGameplay().getFusionCooldownSeconds() * 20;
            boolean appliesCdx = "METAMORU".equals(partnerData.getStatus().getFusionType());
            if (appliesCdx) {
               partnerData.getCooldowns().addCooldown("FusionCooldown", fusionCdTicksx);
            }
            ...
            if (partnerRef != null) {
               ...
               if (partnerRef.hasEffect(MainEffects.FUSED)) {
                  partnerRef.removeEffect(MainEffects.FUSED);
               }

               if (appliesCdx) {
                  partnerRef.addEffect(new MobEffectInstance(MainEffects.FUSION_CD, fusionCdTicksx, 0, false, false, true));
               }
   ...
   private static void calculateAndApplyStats(StatsData l, StatsData p, String type, int lvl1, int lvl2) {
      double ratio = (double)Math.min(lvl1, lvl2) / (double)Math.max(lvl1, lvl2);
      double minMult;
      double maxMult;
      ...
      for (String stat : statsToBoost) {
         ...
         l.getBonusStats().addBonusSplit(stat, "FusionBonus", "+", (double)partnerStatValue * finalMult, true);
      }
   }
   ...
   private static String buildFusionName(String leaderName, String partnerName, String type) {
      if (leaderName == null || leaderName.isEmpty()) {
         ...
         if (cmp == 0) {
            cmp = leaderName.compareToIgnoreCase(partnerName);
         }

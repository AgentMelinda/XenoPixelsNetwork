...
public class KikonoRecipe implements Recipe<KikonoRecipeInput> {
   ...
   public boolean matches(KikonoRecipeInput input, Level level) {
      if (level.isClientSide()) {
         return false;
      } else if (!this.pattern.test(input.getItem(9))) {
         return false;
      } else if (!this.template.test(input.getItem(10))) {
         return false;
      } else {
         for (int i = 0; i < this.recipeItems.size(); i++) {
            if (!((Ingredient)this.recipeItems.get(i)).test(input.getItem(i))) {
               return false;
            }
         }
   ...
   public ItemStack assemble(KikonoRecipeInput input, Provider registries) {
   ...
   public RecipeSerializer<?> getSerializer() {
      return (RecipeSerializer<?>)MainRecipes.KIKONO_SERIALIZER.get();
   }

   public RecipeType<?> getType() {
      return (RecipeType<?>)MainRecipes.KIKONO_TYPE.get();
   }

   public NonNullList<Ingredient> getIngredients() {
      NonNullList<Ingredient> all = NonNullList.create();
      all.addAll(this.recipeItems);
      all.add(this.pattern);
      all.add(this.template);
      return all;
   }
   ...
   public static class Serializer implements RecipeSerializer<KikonoRecipe> {
      public static final KikonoRecipe.Serializer INSTANCE = new KikonoRecipe.Serializer();
      private static final MapCodec<KikonoRecipe> CODEC = RecordCodecBuilder.mapCodec(
         inst -> inst.group(
               ...
               .apply(inst, (output, ingredients, pattern, template, time, energy) -> {
                  ...
                  for (int i = 0; i < Math.min(9, ingredients.size()); i++) {
                     list.set(i, (Ingredient)ingredients.get(i));
                  }

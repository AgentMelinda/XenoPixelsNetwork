...
public class SoundHelper {
   ...
   public static SoundEvent resolveSoundEvent(WeaponAttributes.Sound sound) {
      if (sound != null && sound.id() != null && !sound.id().isBlank()) {
         ResourceLocation soundLoc = ResourceLocation.tryParse(sound.id());
         if (soundLoc == null) {
            soundLoc = ResourceLocation.fromNamespaceAndPath("dragonminez", sound.id());
         }

         return (SoundEvent)BuiltInRegistries.SOUND_EVENT.get(soundLoc);
      } else {
         return null;
      }
   }
   ...
   public static float computePitch(WeaponAttributes.Sound sound) {
      return sound.randomness() > 0.0F ? rng.nextFloat(sound.pitch() - sound.randomness(), sound.pitch() + sound.randomness()) : sound.pitch();
   }
}

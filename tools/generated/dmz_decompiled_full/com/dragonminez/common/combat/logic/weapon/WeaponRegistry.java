...
public class WeaponRegistry {
   ...
   public static WeaponAttributes getAttributes(ItemStack itemStack) {
      if (itemStack == null) {
         ...
         return getAttributes(ResourceLocation.parse("minecraft:air"));
      } else {
         AttributesContainer inStackAttributes = WeaponAttributesHelper.getContainerFromNBT(itemStack);
         if (inStackAttributes != null) {
            if (inStackAttributes.attributes() != null) {
               ...
               WeaponAttributes parentAttributes = getAttributes(parseId(inStackAttributes.parent()));
               if (parentAttributes != null) {
                  return WeaponAttributesHelper.override(parentAttributes, inStackAttributes.attributes());
               }
            } else if (inStackAttributes.parent() != null) {
               return getAttributes(parseId(inStackAttributes.parent()));
            }
         }

         ResourceLocation itemId = BuiltInRegistries.ITEM.getKey(itemStack.getItem());
         return itemId != null ? getAttributes(itemId) : null;
      }
   }

   public static WeaponAttributes getAttributes(ResourceLocation itemId) {
      return itemId == null ? null : registrations.get(itemId);
   }
   ...
   private static ResourceLocation parseId(String id) {
      if (id == null) {
         ...
         ResourceLocation parsed = ResourceLocation.tryParse(id.trim().toLowerCase(Locale.ROOT));
         if (parsed == null) {
            LogUtil.warn(Env.COMMON, "Skipping invalid weapon attribute ResourceLocation: " + id);
         }
   ...
   public static void resolveAndRegisterAttributes(ResourceLocation itemId, AttributesContainer container) {
      WeaponAttributes resolved = resolveAttributes(itemId, container, new HashSet<>());
      if (resolved != null) {
         register(itemId, resolved);
      }
   }
   ...
   private static WeaponAttributes resolveAttributes(ResourceLocation itemId, AttributesContainer container, Set<ResourceLocation> visiting) {
      if (container == null) {
         ...
         ResourceLocation parentId = parseId(container.parent());
         if (parentId == null) {
            return container.attributes();
         } else {
            if (itemId != null) {
            ...
            WeaponAttributes parentAttributes = getAttributes(parentId);
            if (parentAttributes == null && !visiting.contains(parentId)) {
               AttributesContainer parentContainer = containers.get(parentId);
               if (parentContainer != null) {
                  parentAttributes = resolveAttributes(parentId, parentContainer, visiting);
                  if (parentAttributes != null) {
                     register(parentId, parentAttributes);
                  }
               }
            }
            ...
            if (parentAttributes == null) {
               ...
               return container.attributes() == null ? parentAttributes : WeaponAttributesHelper.override(parentAttributes, container.attributes());
            }
         }
      }
   }
   ...
   public static void buildAnimationCache(ResourceManager resourceManager) {
      ...
      Map<ResourceLocation, Resource> resources = resourceManager.listResources("animations", id -> id.getPath().endsWith(".json"));
         ...
         try {
            ...
            if (root.has("animations")) {
               ...
               for (String animName : animations.keySet()) {
                  VALID_ANIMATIONS.add(animName);
               }
            }
         } catch (Exception var9) {
            LogUtil.error(Env.COMMON, "Failed to cache animations from: " + entry.getKey(), var9);
         }
      }
   ...
   public static void loadAttributes(ResourceManager resourceManager) {
      ...
      containers.clear();
      registrations.clear();
      Map<ResourceLocation, Resource> resources = resourceManager.listResources("weapon_attributes", idx -> idx.getPath().endsWith(".json"));
         ...
         try {
            ...
            if (root.has("attributes")) {
               ...
               if (attributes.has("attacks")) {
                  for (JsonElement attackElem : attributes.getAsJsonArray("attacks")) {
            ...
            JsonReader reader = new JsonReader(new StringReader(root.toString()));
            AttributesContainer container = WeaponAttributesHelper.decode(reader);
            if (container != null) {
               String rawPath = id.getPath().substring("weapon_attributes/".length(), id.getPath().length() - ".json".length());
               ResourceLocation itemId = parseId(id.getNamespace() + ":" + rawPath);
               if (itemId != null) {
                  containers.put(itemId, container);
               }
            }
         } catch (Exception var14) {
            LogUtil.error(Env.COMMON, "Failed to load weapon attributes from: " + id, var14);
         }
      }
   ...
   public static String getClosestAnimation(String input) {
      if (VALID_ANIMATIONS.contains(input)) {
         return input;
      } else {
         String cleanInput = input.replace("dragonminez:", "").replace("bettercombat:", "").replace("combat.", "");
   ...
   public static void encodeRegistry() {
      ...
      if (ConfigManager.getCombatConfig().getWeaponRegistryCompression()) {
         ...
         String compressedJson = CompressionHelper.gzipCompress(json);
   ...
   public static void decodeRegistry(String json) {
      ...
      Map<String, WeaponAttributes> readRegistrations = (Map<String, WeaponAttributes>)gson.fromJson(json, mapType);

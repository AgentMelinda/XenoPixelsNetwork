...
public class KiWeaponHelper {
   public static float[] resolveColorForType(String type, float[] kiColor) {
      CombatConfig.KiWeaponConfig cfg = type != null ? ConfigManager.getCombatConfig().getKiWeaponConfig(type) : null;
   ...
   public static float[] resolveColor(String forcedColor, float[] kiColor) {
      if (forcedColor == null) {
         ...
         if (!hex.isEmpty() && !hex.equalsIgnoreCase("#FFFFFF") && !hex.equalsIgnoreCase("FFFFFF")) {
            if (hex.startsWith("#")) {
               hex = hex.substring(1);
            }

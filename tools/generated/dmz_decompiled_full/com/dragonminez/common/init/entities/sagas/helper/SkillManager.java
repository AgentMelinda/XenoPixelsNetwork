...
public class SkillManager {
   private static final Map<Integer, SkillManager.KiAction> REGISTRY = new HashMap<>();
   ...
   public static void execute(int id, DBSagasEntity user, LivingEntity target) {
      SkillManager.KiAction action = REGISTRY.get(id);
      if (action != null) {
   ...
   public static float getCalculatedDamage(int id, DBSagasEntity user) {
      ...
      return switch (id) {
         case 6 -> 0.0F;
         case 7, 12, 19 -> meleeDmg * mult;
         default -> kiDmg * mult;
         case 10, 20 -> kiDmg * mult / 8.0F;
         case 11 -> kiDmg * mult / 4.0F;
         case 13 -> kiDmg * mult / 3.0F;
      };
   }
   ...
   static {
      ...
      REGISTRY.put(
         7,
         (user, target, dmg) -> {
            user.playSound((SoundEvent)MainSounds.OOZARU_GROWL_PLAYER.get(), 2.0F, 0.8F + user.getRandom().nextFloat() * 0.4F);
            if (!user.level().isClientSide && user.level() instanceof ServerLevel serverLevel) {
               double range = 8.0;
               serverLevel.sendParticles(
                  ParticleTypes.EXPLOSION,
                  user.getX(),
                  user.getY() + (double)user.getBbHeight() / 2.0,
                  user.getZ(),
                  100,
                  range / 1.5,
                  range / 1.5,
               ...
               for (LivingEntity entity : serverLevel.getEntitiesOfClass(LivingEntity.class, roarBox)) {
                  if (entity != user && entity.isAlive()) {
                     ...
                     Vec3 push = new Vec3(entity.getX() - user.getX(), 0.5, entity.getZ() - user.getZ()).normalize().scale(3.5);
      ...
      REGISTRY.put(
         ...
         (user, target, dmg) -> {
            ...
            wave.setupKiWave(
               user,
               dmg,
               user.getKiBlastSpeed(),
               user.getCurrentPoolColorMain(),
               user.getCurrentPoolColorBorder(),
               user.getCurrentPoolColorOutline(),
               user.getCurrentPoolSkillSize(),
               37
            );
         }
      );
      REGISTRY.put(
         ...
         (user, target, dmg) -> {
            ...
            oozaru.setupKiOozaru(
               user, dmg, user.getKiBlastSpeed(), user.getCurrentPoolColorMain(), user.getCurrentPoolColorBorder(), user.getCurrentPoolSkillSize(), 37
            );
         }
      );
      ...
      REGISTRY.put(11, (user, target, dmg) -> {
         ...
         user.playSound((SoundEvent)MainSounds.KIBLAST_ATTACK.get(), 1.0F, 1.0F + user.getRandom().nextFloat() * 0.2F);
      });
      ...
      REGISTRY.put(
         ...
         (user, target, dmg) -> {
            ...
            doubleSunday.setupDoubleSunday(
               user,
               dmg,
               user.getKiBlastSpeed(),
               user.getCurrentPoolColorMain(),
   ...
   public interface KiAction {
      void execute(DBSagasEntity var1, LivingEntity var2, float var3);
   }
}

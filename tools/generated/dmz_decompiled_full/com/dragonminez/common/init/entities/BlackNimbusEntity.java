...
public class BlackNimbusEntity extends Mob implements GeoEntity {
   ...
   public void tick() {
      ...
      if (this.level().isClientSide) {
         this.spawnAuraParticles(2567527);
      }
   }
   ...
   protected InteractionResult mobInteract(Player player, InteractionHand hand) {
      if (!this.level().isClientSide) {
         int alignment = StatsProvider.get(StatsCapability.INSTANCE, player).map(data -> data.getResources().getAlignment()).orElse(100);
         if (alignment >= 67) {
            player.displayClientMessage(Component.translatable("message.dragonminez.black_nimbus.not_evil"), true);
   ...
   public boolean hurt(DamageSource pSource, float pAmount) {
      if ("player".equals(pSource.getMsgId()) && pSource.getEntity() instanceof Player) {
         if (!this.level().isClientSide && this.isAlive()) {
            this.spawnAtLocation((ItemLike)MainItems.NUBE_NEGRA_ITEM.get());

...
public class PothalaPairItem extends Item {
   ...
   public void inventoryTick(ItemStack stack, Level level, Entity entity, int slotId, boolean isSelected) {
      if (!level.isClientSide) {
         ensurePairId(stack, level.random);
      }
   }
   ...
   public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
      ...
      if (!level.isClientSide) {
         ...
         ItemStack left = new ItemStack((ItemLike)this.leftEarring.get());
         ItemStack right = new ItemStack((ItemLike)this.rightEarring.get());
   ...
   public static void setPairId(ItemStack stack, int id) {
      CustomData.update(DataComponents.CUSTOM_DATA, stack, tag -> tag.putInt("PothalaPairId", id));
   }
   ...
   public static int getPairId(ItemStack stack) {
      ...
      return tag.contains("PothalaPairId") ? tag.getInt("PothalaPairId") : 0;
   }

   public static void appendPairIdTooltip(ItemStack stack, List<Component> tooltip) {
      int pairId = getPairId(stack);
      if (pairId != 0) {
         tooltip.add(Component.translatable("item.dragonminez.pothala.pair_id", new Object[]{String.format("%08X", pairId)}).withStyle(ChatFormatting.GRAY));
      }
   }
}

...
public class SpacePodDestinationRegistry extends SimpleJsonResourceReloadListener {
   ...
   protected void apply(Map<ResourceLocation, JsonElement> map, ResourceManager resourceManager, ProfilerFiller profiler) {
      List<Entry<ResourceLocation, JsonElement>> entries = new ArrayList<>(map.entrySet());
      entries.sort(Comparator.comparing(entryx -> ((ResourceLocation)entryx.getKey()).toString()));
      List<SpacePodDestinationDefinition> loaded = new ArrayList<>();

      for (Entry<ResourceLocation, JsonElement> entry : entries) {
         ...
         for (JsonElement element : GsonHelper.getAsJsonArray(root, "destinations")) {
   ...
   private static JsonElement unlockToJson(SpacePodUnlockExpression expression) {
      if (expression instanceof SpacePodUnlockExpression.Primitive primitive) {
      ...
      } else if (expression instanceof SpacePodUnlockExpression.QuestCompleted questCompleted) {
         JsonObject object = new JsonObject();
         object.addProperty("quest", questCompleted.questId());
         return object;
      } else if (expression instanceof SpacePodUnlockExpression.LevelAtLeast levelAtLeast) {
         JsonObject object = new JsonObject();
         object.addProperty("level", levelAtLeast.level());
         return object;
      } else if (expression instanceof SpacePodUnlockExpression.StatAtLeast statAtLeast) {
         JsonObject object = new JsonObject();
         ...
         return object;
      } else if (expression instanceof SpacePodUnlockExpression.RaceMatches raceMatches) {
         JsonObject object = new JsonObject();
         object.addProperty("race", raceMatches.race());
         return object;
      } else if (expression instanceof SpacePodUnlockExpression.PlayerTag playerTag) {
         JsonObject object = new JsonObject();
         object.addProperty("tag", playerTag.tag());
         return object;
      } else if (expression instanceof SpacePodUnlockExpression.VisitedDimension visitedDimension) {
         JsonObject object = new JsonObject();
         object.addProperty("visited_dimension", visitedDimension.dimensionId());
         return object;
      } else if (expression instanceof SpacePodUnlockExpression.And andExpression) {
         JsonObject object = new JsonObject();
         JsonArray array = new JsonArray();
         ...
         return object;
      } else if (!(expression instanceof SpacePodUnlockExpression.Or orExpression)) {

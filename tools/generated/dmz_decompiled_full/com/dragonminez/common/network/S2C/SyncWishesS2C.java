...
public class SyncWishesS2C {
   ...
   public void encode(FriendlyByteBuf buf) {
      buf.writeByteArray(this.compressedWishes);
   }

   public void handle(Supplier<NetworkEvent.Context> ctx) {
      ctx.get().enqueueWork(() -> DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> {
               String decompressedJson = CompressionUtil.decompress(this.compressedWishes);
               Type mapType = (new TypeToken<Map<String, List<Wish>>>() {
               }).getType();
               Map<String, List<Wish>> wishes = (Map<String, List<Wish>>)GSON.fromJson(decompressedJson, mapType);
               if (wishes != null) {

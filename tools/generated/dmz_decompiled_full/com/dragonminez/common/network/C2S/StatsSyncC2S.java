            ...
            () -> {
               ...
               if (player != null) {
                  if (!ConfigManager.isRaceLoaded(msg.raceName)) {
                     LogUtil.warn(Env.COMMON, "Rejected StatsSyncC2S from '{}': unknown race '{}'", player.getGameProfile().getName(), msg.raceName);
                     StatsProvider.get(StatsCapability.INSTANCE, player)
                        .ifPresent(data -> NetworkHandler.sendToTrackingEntityAndSelf(new StatsSyncS2C(player), player));
                  } else {
                     StatsProvider.get(StatsCapability.INSTANCE, player).ifPresent(data -> {
                        ...
                        if (!data.getStatus().isHasCreatedCharacter()) {
                           ...
                           if (ConfigManager.getRaceCharacter(msg.raceName) != null) {
                              character.setHasSaiyanTail(ConfigManager.getRaceCharacter(msg.raceName).getHasSaiyanTail());
                           }

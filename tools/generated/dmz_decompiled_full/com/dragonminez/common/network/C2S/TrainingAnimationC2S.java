...
public class TrainingAnimationC2S {
   private static final String[] TRAINING_ANIMATIONS = new String[]{"base.flex", "base.meditation"};
      ...
      ctx.get()
         .enqueueWork(
            () -> {
               ...
               if (player != null) {
                  if (this.active) {
                     boolean canTrain = StatsProvider.get(StatsCapability.INSTANCE, player).map(data -> data.getStatus().isHasCreatedCharacter()).orElse(false);
                     if (!canTrain) {
                        return;
                     }

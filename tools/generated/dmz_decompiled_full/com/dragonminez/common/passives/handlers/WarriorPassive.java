...
public class WarriorPassive implements IClassPassive {
   ...
   public double staminaRegenMultiplier(StatsData data) {
      return 1.0 + (double)this.stacks(data) * ClassPassives.value(data, "stmRegenPerStack", 0.1);
   }

   @Override
   public double armorPenBonus(StatsData data) {
      int maxStacks = (int)ClassPassives.value(data, "maxStacks", 5.0);
      return this.stacks(data) >= maxStacks ? ClassPassives.value(data, "armorPenAtMax", 0.1) : 0.0;
   }
   ...
   public void onPlayerTick(ServerPlayer player, StatsData data) {
      ...
      if (state != null) {
         ...
         if (state.warriorStacks > 0 && now > state.warriorStacksExpireTick) {
            state.warriorStacks = 0;
         }

         if (state.warriorComboProgress > 0 && now > state.warriorComboExpireTick) {
            state.warriorComboProgress = 0;
         }
      }
   }

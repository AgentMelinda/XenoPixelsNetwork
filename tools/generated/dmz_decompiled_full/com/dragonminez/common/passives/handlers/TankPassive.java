...
public class TankPassive implements IClassPassive {
   ...
   private boolean isLowHp(StatsData data) {
      if (data.getPlayer() != null && !(data.getPlayer().getMaxHealth() <= 0.0F)) {
         double ratio = (double)(data.getPlayer().getHealth() / data.getPlayer().getMaxHealth());
         return ratio < ClassPassives.value(data, "lowHpThreshold", 0.3);
      } else {
         return false;
      }
   }
   ...
   public double bonusHpRegenFromStamina(StatsData data) {
      ...
      if (this.isLowHp(data)) {
         bonus *= ClassPassives.value(data, "lowHpMultiplier", 2.0);
      }

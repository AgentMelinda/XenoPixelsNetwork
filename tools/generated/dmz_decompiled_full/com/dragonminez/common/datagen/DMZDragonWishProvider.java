...
public class DMZDragonWishProvider implements DataProvider {
   ...
   public DMZDragonWishProvider(PackOutput output) {
      this.output = output;
   }
   ...
   public CompletableFuture<?> run(CachedOutput cachedOutput) {
      ...
      for (DragonDefinition dragon : DragonBallDefinitions.getBootstrapDragons()) {
         String var6 = dragon.getId();

         List<Wish> wishes = switch (var6) {
            case "shenron" -> buildShenron();
            case "porunga" -> buildPorunga();
            default -> List.of();
         };
         if (dragon.getBallSetId() != null) {
            future = CompletableFuture.allOf(future, this.save(cachedOutput, dragon, wishes));
         }
      }
   ...
   private CompletableFuture<?> save(CachedOutput cachedOutput, DragonDefinition dragon, List<Wish> wishes) {
      ...
      Path path = this.output
         .getOutputFolder(Target.DATA_PACK)
         .resolve("dragonminez")
         .resolve("dragonballs")
         .resolve(dragon.getBallSetId())
         .resolve("definitions")
         .resolve("wishes.json");
   ...
   private static List<Wish> buildPorunga() {
      ...
      potaraYellow.add(new GenericItemDTO("dragonminez:pothala_left", 1));
      potaraYellow.add(new GenericItemDTO("dragonminez:pothala_right", 1));
      ...
      potaraGreen.add(new GenericItemDTO("dragonminez:green_pothala_left", 1));
      potaraGreen.add(new GenericItemDTO("dragonminez:green_pothala_right", 1));
      defaultWishes.add(new ItemListWish("wish.porunga.pothala_green.name", "wish.porunga.pothala_green.desc", potaraGreen));
      return defaultWishes;
   }

   public String getName() {
      return "DragonMineZ dragonballs wish datapack provider";
   }
}

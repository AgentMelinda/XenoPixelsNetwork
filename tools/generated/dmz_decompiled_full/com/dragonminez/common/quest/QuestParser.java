...
public class QuestParser {
   private static final Gson GSON = new GsonBuilder().registerTypeAdapter(GenericItemDTO.class, new GenericItemTypeAdapter()).setPrettyPrinting().create();
   ...
   public static Quest parseQuest(JsonObject json) {
      if (json != null && json.has("id") && json.has("title") && json.has("type")) {
         ...
         if (numericId != -1 || stringId != null && !stringId.isBlank()) {
            ...
            String questGiver = json.has("quest_giver") && !json.get("quest_giver").isJsonNull() ? json.get("quest_giver").getAsString() : null;
            String turnIn = json.has("turn_in") && !json.get("turn_in").isJsonNull() ? json.get("turn_in").getAsString() : null;
            boolean secret = json.has("secret") && json.get("secret").getAsBoolean();
            Quest.ClaimMode claimMode = parseClaimMode(
               json.has("claim_mode") && !json.get("claim_mode").isJsonNull() ? json.get("claim_mode").getAsString() : null
            );
   ...
   private static QuestPrerequisites parseConditionsBlock(JsonObject json, String key) {
      return json.has(key) && json.get(key).isJsonObject() ? parsePrerequisites(json.getAsJsonObject(key)) : null;
   }
   ...
   public static List<QuestObjective> parseObjectiveList(JsonObject questJson) {
      ...
      if (!questJson.has("objectives")) {
         ...
         for (JsonElement element : questJson.getAsJsonArray("objectives")) {
   ...
   public static QuestObjective parseObjective(JsonObject json) {
      if (json != null && json.has("type")) {
         ...
         String var2 = type.toUpperCase();

         return (QuestObjective)(switch (var2) {
            case "ITEM" -> {
            ...
            case "KILL" -> {
               ...
               KillObjective.SpawnMode spawnMode = parseKillSpawnMode(
                  json.has("spawn") && !json.get("spawn").isJsonNull() ? json.get("spawn").getAsString() : null
               );
               KillObjective.CountMode countMode = parseKillCountMode(
                  json.has("count_mode") && !json.get("count_mode").isJsonNull() ? json.get("count_mode").getAsString() : null
               );
               int textureVariant = json.has("TextureVariant") && !json.get("TextureVariant").isJsonNull() ? json.get("TextureVariant").getAsInt() : -1;
               int aiTier = json.has("AITier") && !json.get("AITier").isJsonNull() ? json.get("AITier").getAsInt() : -1;
               boolean canTransform = !json.has("canTransform") || json.get("canTransform").isJsonNull() || json.get("canTransform").getAsBoolean();
               ...
               yield new KillObjective(
                  ...
                  canTransform,
                  transformHealth,
                  transformMeleeDamage,
                  transformKiDamage,
                  transformHealthMultiplier,
                  transformMeleeMultiplier,
                  transformKiMultiplier,
            ...
            case "INTERACT" -> {
               ...
               EntityType<?> interactType = interactEntity != null
                  ? (EntityType)BuiltInRegistries.ENTITY_TYPE.get(ResourceLocation.parse(interactEntity))
                  : null;
   ...
   public static List<QuestReward> parseRewardList(JsonObject questJson) {
      ...
      if (!questJson.has("rewards")) {
         ...
         for (JsonElement element : questJson.getAsJsonArray("rewards")) {
   ...
   public static QuestReward parseReward(JsonObject json) {
      if (json != null && json.has("type")) {
         ...
         String var4 = type.toUpperCase();

         QuestReward reward = (QuestReward)(switch (var4) {
            case "ITEM" -> {
               String itemId = json.get("item").getAsString();
            ...
            case "GENERIC_ITEM" -> {
               GenericItemDTO genericItem = (GenericItemDTO)GSON.fromJson(json.getAsJsonObject("itemReward"), GenericItemDTO.class);
            ...
            case "TRANSFORMATION" -> {
               ...
               if (formGroup != null && formName != null) {
   ...
   private static Set<Difficulty> parseRewardDifficulties(JsonObject json) {
      ...
      if (element != null && !element.isJsonNull()) {
         ...
         if (element.isJsonArray()) {
            for (JsonElement token : element.getAsJsonArray()) {
               if (token != null && !token.isJsonNull()) {
                  addDifficultyTokens(result, token.getAsString());
               }
            }
         ...
         return (Set<Difficulty>)(result.isEmpty() ? EnumSet.allOf(Difficulty.class) : result);
      } else {
   ...
   private static QuestPrerequisites.Condition parseCondition(JsonObject json) {
      if (json.has("operator")) {
         ...
         return switch (type) {
            case "SAGA_QUEST" -> QuestPrerequisites.Condition.sagaQuest(json.get("sagaId").getAsString(), json.get("questId").getAsInt());
            case "QUEST" -> QuestPrerequisites.Condition.quest(json.get("questId").getAsString());
            case "STAT" -> QuestPrerequisites.Condition.stat(json.get("stat").getAsString().toUpperCase(), json.get("minValue").getAsInt());
            ...
            case "STRUCTURE" -> QuestPrerequisites.Condition.structure(json.get("structure").getAsString(), parseStructureHint(json));
            ...
            json.has("min") ? json.get("min").getAsInt() : null, json.has("max") ? json.get("max").getAsInt() : null
         );
            case "SKILL" -> {
               String skill = firstString(json, "skill", "skillId", "id");
               yield skill != null ? QuestPrerequisites.Condition.skill(skill, firstInt(json, 1, "minLevel", "level", "required")) : null;
            }
            case "RACE" -> {
   ...
   private static QuestPrerequisites.StructureHint parseStructureHint(JsonObject json) {
      if (json.has("hint") && json.get("hint").isJsonObject()) {
         ...
         String dimensionId = hintJson.has("dimension") && !hintJson.get("dimension").isJsonNull() ? hintJson.get("dimension").getAsString() : null;
         ...
         return dimensionId == null && x == null && y == null && z == null ? null : new QuestPrerequisites.StructureHint(dimensionId, x, y, z);
      } else {
         return null;
      }
   }
      ...
      if (json != null) {
         ...
         if (json.has("objectives") && json.get("objectives").isJsonArray()) {
            ...
            for (JsonElement element : json.getAsJsonArray("objectives")) {
               if (element.isJsonObject()) {
                  validateObjective(source, file, "objectives[" + i + "]", element.getAsJsonObject());
               }
         ...
         if (json.has("rewards") && json.get("rewards").isJsonArray()) {
            ...
            for (JsonElement element : json.getAsJsonArray("rewards")) {
               if (element.isJsonObject()) {
                  validateReward(source, file, "rewards[" + i + "]", element.getAsJsonObject());
               }
   ...
   private static void validateObjective(String source, String file, String path, JsonObject json) {
      ...
      String var6 = type == null ? "" : type;
      Set<String> allowed;
      switch (var6) {
         case "ITEM":
            allowed = JsonKeys.of("type", "item", "count");
            break;
         case "KILL":
            allowed = JsonKeys.of(
               "type",
               ...
               "TransformKiDamage",
               "TransformHealthMultiplier",
               "TransformMeleeDamageMultiplier",
               "TransformKiMultiplier",
               "TransformTriggerPercent"
            );
            break;
         case "BIOME":
            allowed = JsonKeys.of("type", "biome");
            break;
         case "DIMENSION":
            allowed = JsonKeys.of("type", "dimension");
            break;
         case "COORDS":
            allowed = JsonKeys.of("type", "x", "y", "z", "radius");
            break;
         case "INTERACT":
            allowed = JsonKeys.of("type", "entity", "entityName");
            break;
         case "STRUCTURE":
            allowed = JsonKeys.of("type", "structure");
            break;
         case "DRAGON_SUMMON":
            allowed = JsonKeys.of("type", "dragon", "dragon_id", "dragonId", "ball_set", "ballSet", "ball_set_id", "ballSetId", "set");
            break;
         case "TALK_TO":
            allowed = JsonKeys.of("type", "npcId");
            break;
         case "SKILL":
            allowed = JsonKeys.of("type", "skill", "skillId", "id", "level", "minLevel", "required");
            break;
         default:
            JsonKeys.reportBadType(source, file, path, type);
            return;
      }
   ...
   private static void validateReward(String source, String file, String path, JsonObject json) {
      ...
      String var7 = type == null ? "" : type;
      Set<String> allowed;
      switch (var7) {
         case "ITEM":
            allowed = JsonKeys.union(common, "item", "count");
            break;
         case "GENERIC_ITEM":
            allowed = JsonKeys.union(common, "itemReward", "itemType", "itemId", "count", "enchantments", "potion", "mobEffects", "material", "pattern");
            break;
         case "TPS":
            allowed = JsonKeys.union(common, "amount");
            break;
         case "ALIGNMENT":
            allowed = JsonKeys.union(common, "amount");
            break;
         case "COMMAND":
            allowed = JsonKeys.union(common, "command", "translationKey");
            break;
         case "SKILL":
            allowed = JsonKeys.union(common, "skill", "level");
            break;
         case "TRANSFORMATION":
            allowed = JsonKeys.union(common, "formGroup", "form_group", "group", "formName", "form_name", "form", "mastery", "stack");
            break;
         case "KI_TECHNIQUE":
            allowed = JsonKeys.union(common, "code", "techniqueCode", "technique_code");
            break;
         default:
            JsonKeys.reportBadType(source, file, path, type);
            return;
   ...
   private static void validateConditionArray(String source, String file, String path, JsonObject block) {
      if (block.has("conditions") && block.get("conditions").isJsonArray()) {
         int i = 0;

         for (JsonElement element : block.getAsJsonArray("conditions")) {
            if (element.isJsonObject()) {
               validateCondition(source, file, path + ".conditions[" + i + "]", element.getAsJsonObject());
            }
   ...
   private static void validateCondition(String source, String file, String path, JsonObject json) {
      if (json.has("operator")) {
         ...
         switch (var6) {
            case "SAGA_QUEST":
               allowed = JsonKeys.of("type", "sagaId", "questId");
               break;
            case "QUEST":
               allowed = JsonKeys.of("type", "questId");
               break;
            case "STAT":
               allowed = JsonKeys.of("type", "stat", "minValue");
               break;
            case "LEVEL":
               allowed = JsonKeys.of("type", "minLevel");
               break;
            case "BIOME":
               allowed = JsonKeys.of("type", "biome");
               break;
            case "STRUCTURE":
               validateStructureCondition(source, file, path, json);
               return;
            case "DIMENSION":
               allowed = JsonKeys.of("type", "dimension");
               break;
            case "TIME":
               allowed = JsonKeys.of("type", "mode", "ticks", "milliseconds");
               break;
            case "ALIGNMENT":
               allowed = JsonKeys.of("type", "min", "max");
               break;
            case "SKILL":
               allowed = JsonKeys.of("type", "skill", "skillId", "id", "minLevel", "level", "required");
               break;
            case "RACE":
               allowed = JsonKeys.of("type", "race", "raceName", "race_name");
               break;
            case "CLASS":
               allowed = JsonKeys.of("type", "class", "className", "class_name", "characterClass");
               break;
            default:
               JsonKeys.reportBadType(source, file, path, type);
               return;
